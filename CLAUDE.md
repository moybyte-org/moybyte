...
- `firmware/guition_jc3248w535/` — the **third board** (#202): the ~$15 Guition 3.5″ 320×480 S3 smart display, the port-kit's first walk of the six-stage checklist (provisioned + console-on-glass in one session, 2026-08-18 — its on-glass suite `tests/test_guition_on_glass.py` passed 10/10 that night, both cart runtimes included). New panel class: QSPI AXS15231B via `native/moy_axs` (raw spi_master — the whole frame ships under ONE CS assertion, which is why it does not share `moy_lcd`'s C body; the band/bounce/kick-pump-drain DESIGN transferred verbatim). Touch-only, portrait, and the first fullscreen-tier board where system canvas (320×480) ≠ game canvas (320×240) — carried entirely by the existing `SystemCanvas` + `wm.composite_game`, zero new shared code. Read that dir's README before touching it; #202 tracks the open tail (touch calibration, SD stage 4, audio stage 5, a touch-only game-exit gesture).
