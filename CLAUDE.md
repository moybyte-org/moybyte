...
### Device module map (`firmware/lilygo_t_deck_plus_mainline/modules/`)
...
- **T-Deck USB under the desktop — the 2026-07-10 measured verdict (replaces the old "starves USB" lore).** Serial **TX streams fine during play** (PERF/MEMX/HITCH lines flow for hours — passively read `/dev/ttyACM*` with reconnect) and **esptool flashing works** — but its auto-reset never syncs on this board's native USB, so the board has to be put in the ROM loader BY HAND: there is no BOOT button — **the trackball click is GPIO0, so hold the trackball in while powering the board on**, then let go (the P4's CH343 auto-resets fine). **RX is dead on the shipped loop, and the cause is UPSTREAM, not our stack** (root-caused 2026-08-16; the previous explanation here — "this fork's USB-CDC stack has no at-arrival interrupt-char scan" — was WRONG, and `tud_cdc_rx_cb` does scan and is linked into the image). **MicroPython only initialises TinyUSB CDC when the REPL is reached** (micropython/micropython#18581, a v1.27.0 regression; v1.26.1 was fine). Our console never returns to the REPL, so CDC is never brought up and we are talking to the USB-Serial-JTAG interface instead — which carries TX and whose input nothing reads.

The tell is the USB product id, and it is decisive: **`303a:1001` = no REPL reached, RX dead; `303a:4001` = TinyUSB CDC up, RX works.** Measured on this board the same night: the mainline port's stage-1 panel smoke RETURNS to the REPL, enumerated `4001`, and answered typed commands; the full console takes the loop, stays on `1001`, and a host `write()` times out having transferred **0 bytes** — the OUT endpoint is never drained, so nothing reaches `sys.stdin` to be parsed. That also kills the second-guess (`readline()` blocking per character): there is nothing to block on.

So the two reverted in-loop experiments failed for a reason neither of them could have fixed, and **rebuilding on mainline changes nothing by itself** — v1.28 still ties CDC init to the REPL. The fix is to bring USB CDC up at BOOT independently of the REPL (make the console TinyUSB CDC in sdkconfig rather than USB-Serial-JTAG, or init it explicitly before the desktop takes the loop), and it is worth real effort: it would give the T-Deck the P4's whole on-glass harness. The P4 is unaffected — its CH343 is a separate USB-UART chip, not the S3's native USB.
