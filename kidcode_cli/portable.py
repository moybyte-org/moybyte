...
ALLOWED_IMPORTS = ["kidcode", "math", "random"]
DISALLOWED_CALLS = [
    "__import__",
    "compile",
    "dir",
                ...
                if not _is_allowed_import(alias.name):
                    issues.append(
                        PortableIssue(
                            path,
                            node.lineno,
                            "import '" + alias.name + "' is not in the portable KidCode subset",
                        )
                    )
            ...
            if node.level:
                ...
                issues.append(
                    PortableIssue(
                        path,
                        node.lineno,
                        "import from '" + module + "' is not in the portable KidCode subset",
                    )
                )
            ...
            if name in DISALLOWED_CALLS:
                issues.append(
                    PortableIssue(
                        path,
                        node.lineno,
                        "call to '" + name + "' is not in the portable KidCode subset",
                    )
                )
    return issues
