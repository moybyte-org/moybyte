...
BUNDLE_SCHEMA = "kidcode.bundle.v1"
...
def default_bundle_path(project_path):
    root = project_path[:-7] if project_path.endswith(".kcproj") else project_path
    return root + ".kc8"
...
def pack_project(project_path, out_path=None, include_generated=False):
    ...
    with zipfile.ZipFile(out_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        archive.writestr("kidcode_bundle.json", json.dumps(metadata, indent=2) + "\n")
        for rel in files:
