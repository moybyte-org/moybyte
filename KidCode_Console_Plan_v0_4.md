# KidCode Console / Spryte OS — Plan v0.4

**Project:** Kids Edu Slate / Laptop → KidCode Console / Spryte OS / KidCode Fantasy Workstation  
...
v0.3 used a KidCode-8 style 128×128 canvas.
...
**KidCode Console is a physical fantasy workstation for kids: a chunky keyboard-first creative computer where the desktop, tools, games, apps, art, music, lessons, and widgets are editable cartridges.**
...
But keep `.kcart` internally for now.
...
```text
Lua spr()       → native kc_canvas_sprite()
Python kid.spr → native kc_canvas_sprite()
```
...
1. Final internal name: KidCode OS vs Spryte OS vs other.
...
6. Whether `.kcart` remains the public extension.
...
6. Cloud AI business model.
7. Final industrial design.
8. Marketplace/library strategy.
...
```text
KidCode/Spryte is a physical kid-safe fantasy workstation.
