...
> **DECIDED — Moybyte is "learn to make," not "learn to use."** The flagship **One** is the make-workstation and the identity anchor. **Zero** and **Player** are reach and play *around* that core — the funnel that pulls a kid from playing carts to making them. "Learning computer use" is the Trojan horse, not the reason the product exists.
...
| One "fantasy workstation" device | A **three-tier hardware family — Zero / Player / One** — over one shared console; tiers differ only in backend | #59 |
...
> **One console, N backends. A tier is a backend port, not a fork.** The shared `console` / `editors` / `moy_carts` modules, the cart format, and the **320×240 indexed MOY64 canvas contract** are identical across every tier. Only the display / input / storage / transport backend differs. **The cart contract is the portability guarantee — it is the product's technical moat.**
...
- **"Learn programming"** is a *creation / agency* play — give the kid a workshop. It competes with micro:bit, PICO-8, Scratch, Kano; it doesn't fight consumption on fun-per-minute, it competes on **pride-of-making** and identity ("I'm someone who makes things"). Smaller audience, but stickier, clearer moat, higher price tolerance, and a parent who *actively chooses* it. **This is us.**
...
## 3. The three-tier hardware family (#59)
...
| **Price — launch (~1k, DTC)** | ~$15–19 | ~$50 | ~$130–160 |
| **Price — at volume (10k+, DTC)** | ~$10–12 | ~$35–40 | ~$100–150 |
| **Status** | design (enabled by #41/#22) | **shipping** (LilyGO T-Deck class) | porting (#58) |

*(BOM-grounded 2026-07 — see §3.5 and `docs/moybyte_BOM_pricing_2026-07.md`. BOMs: Zero ~$8, Player ~$25, One ~$64 + tooling.)*

**Why a tier is cheap: it's a backend port, not a product.** The console UI, editors, cart store, and canvas are one codebase. Zero and Player ride **off-the-shelf** boards (they prove portability and seed a community); **the One is the bespoke flagship — the first product we actually design and ship.**

### 3.2 The One — the make-workstation (the flagship, #58)
...
The One's value prop, stated crisply: **"make without a tether — standalone, big screen, real keys."**
...
### 3.5 Price ladder & the create/consume funnel (DECIDED direction)

- **Do not force the flagship to $60.** A 7″ battery+keyboard device can't hit $60 retail without selling at/below cost (retail ≈ 2–3× BOM, and Moybyte has *no* recurring revenue by design to subsidize it). The tier family sidesteps this: the **Player** is the sub-$50 accessible tier, the **One** is allowed to be a $100–150 *desirable object* (Playdate logic), and the **Zero** is the reach tier.
- **BOM reality (grounded 2026-07 — `docs/moybyte_BOM_pricing_2026-07.md`).** At ~1k launch volume, DTC prices land higher than the aspiration: **Zero ~$15–19** (BOM ~$8), **Player ~$50** (BOM ~$25), **One ~$130–160** (BOM ~$64 + tooling). Two hard constraints fall out:
  - **(a) Sell direct-to-consumer.** A 3× retail-channel markup makes *every* tier's target impossible ($23 / $74 / $193) — no Amazon/distributor shelf; the bands only work DTC (own store / Kickstarter). This *reinforces* the §2.3 positioning: the DTC/maker channel is the one that fits "learn to make" anyway.
  - **(b) Volume is the lever.** The target bands (Zero ~$10, Player ~$35–40, One ~$100–150) only appear at **10k+ units**, and the One's injection-mold tooling (~$15–40k/part; ~$30–80/unit at 1k, ~$3–8 at 10k) is economically insane below that. So the sequencing below isn't a preference — it's numerically forced.
  - The **$10 Zero is a subsidized loss-leader**, not a launch price — it only pencils once One/Player revenue funds it (the Pi-Zero halo-down model).
- **Price down over time, not at birth.** Launch the One to early-adopter/maker parents; drive a stripped "school edition" toward $60 at volume later. *(The $60 school edition is **EXPLORATION / OPEN**, not a committed SKU.)*
- **The create/consume axis and the price ladder run opposite — on purpose.** Authoring (keyboard) lives in the One; the cheap tiers *play*. That's a **funnel** (play cheap → graduate to making), not a bug. And it's softened by web-view authoring (§8.4): a $10 Zero + the kid's own phone = a real authoring station, so *making is available on every tier* (tethered), while the One is where making is **best and untethered**.
- **Tiers are roles, not just price points.** In a local multiplayer session (§9), the **One is the table/host** (big screen, most RAM) and **Players/Zeros are the controllers**. A living-room party where the flagship is the board and the handhelds are the pads.
...
The game canvas works in **palette indices** (the **MOY64** palette) with a plain-function drawing API — no dependency on `framebuf`, LVGL, or even Python. This is deliberate: the *same* `.moy` runs on the host, on every device tier (indices → RGB565 via `moy_compositor`), and eventually a Lua VM. **This is the moat. Hold the line at 320×240 for carts.**
...
This is what makes the create/consume funnel humane: a $10 Zero + a phone the kid already owns = a genuine authoring station. The One stays differentiated as *untethered, standalone, best-ergonomics* making.
...
### 11.5 Manufacturing strategy — off-the-shelf + 3D-print first, tool for volume later (DECIDED direction)

The BOM analysis (§3.5) showed the One is uneconomic at low volume for one reason: **injection-mold tooling** (~$15–40k/part, only amortizing at 10k+ units) plus custom-PCB/MIPI-DSI engineering (NRE). The launch path sidesteps both with a deliberate two-phase plan:

**Phase 1 — validation run (low volume, ≈zero capex).**
- **Electronics: an off-the-shelf ESP32-P4 board as the product core** — lead with the **Guition JC1060P470C (~$35)** (Waveshare ESP32-P4-WIFI6-Touch-LCD-7B ~$80 as fallback). Eliminates custom-PCB NRE + display sourcing + high-speed DSI board design (hard, expensive, error-prone). #58 already targets porting to these boards, so the firmware path is the same.
- **Enclosure: 3D-printed (FDM).** Eliminates the tooling gamble entirely — build 50 or 500 units and iterate the design between them. On-brand bonus: **open-source the shell STLs** so buyers print/remix their own case — "make it yours," extended to the hardware.
- **Honest trade-off:** per-unit cost goes *up* (printed shell ~$15–40 vs ~$6 molded; plus the dev board's margin), and the feel is "maker," not "premium." But the launch audience (Kickstarter / early adopters / makers) reads that as charming, and it converts the One from *"needs 10k units to exist"* into *"can ship 100 to early adopters."* Maker-priced (~$150).

**Phase 2 — cost-down at volume (10k+).** Custom PCB + injection-molded, kid-proof enclosure, once demand is proven and tooling amortizes — reaching the $100–150 target band and a premium shell.

This is a *sequence*, not a preference: **Phase 1 funds the volume that justifies Phase 2.** It also defuses the §15 volume question — there is no up-front 10k gamble. (The Player can follow the same pattern: off-the-shelf S3 handheld → 3D-printed shell → custom S3 PCB later.)
...
1. **The One / P4 port (#58)** — the flagship backend; de-risk toolchain → native DSI driver → wire the shared console.
...
1. **Production volume & funding model (#59).** BOMs are now grounded (§3.5; `docs/moybyte_BOM_pricing_2026-07.md`) — launch-DTC ≈ $17 / $50 / $145. The open lever is **volume**: the target bands and the One's tooling only pencil out at 10k+ units, and channel sales never do. **Phase-1 manufacturing (§11.5 — off-the-shelf P4 board + 3D-printed shell) removes the up-front tooling gamble, so a small validation run is viable without committing to 10k.** Still to decide: the funding path (Kickstarter? pre-order?) that gets from a validation run to the Phase-2 volume.
2. **Tagline & trademark (#23).** Brand is **Moybyte** (rename merged); the device line is **Zero / Player / One** (DECIDED 2026-07-01 — familiar-but-clear, `Pi`-style ladder). Remaining: the tagline (the "MOY = *moj* / *my* computer" narrative) and domain/trademark diligence.
3. **TIC-80 canvas reconciliation (#11).** 240×136 sub-viewport vs a 320×240 compatible subset.
4. **The Player's final board** — off-the-shelf S3 handheld (confirm PSRAM) vs a light custom S3 PCB.

**Can decide later**
5. **$60 "school edition"** (EXPLORATION) — a stripped volume SKU for the institutional/network play.
6. **Commit to system-as-carts (#55)** — needs the privilege boundary + crash isolation first.
7. **Rich web-view 3D scope (EXPLORATION, §10.3)** — if/when, and only in the fenced "device-as-server" form.
8. **Lua second cart type timing (#11)** — gated on a real `.tic`/Lua source-compat goal.
9. **Lego/modules scope (#9)** and **BLE input (#26)** — the Hardware Lab surface.
10. **AI helper (#8)** — gateway, provider, business model.
...
The One is the flagship and the identity anchor; the cheap tiers are reach and play — the funnel into making.
