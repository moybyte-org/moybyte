"""Runtime context for KidCode projects."""
...
from .errors import KidCodeRuntimeError, friendly_from_exception
...
def current_context():
    if _current is None:
        raise RuntimeError("KidCode runtime is not active")
    return _current
...
class RuntimeContext:
    ...
    def load_entry(self, entry_path):
        ...
        try:
            ...
            runpy.run_path(entry_path, run_name="__kidcode_project__")
            self.state = "READY"
        except Exception as exc:
            self.error = friendly_from_exception(exc, entry_path)
            self.state = "ERROR"
            raise KidCodeRuntimeError(self.error) from exc
        finally:
            sys.path = old_path
            os.chdir(old_cwd)
