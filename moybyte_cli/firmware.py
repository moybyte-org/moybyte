...
from moybyte_cli.boards import get_board_profile
...
def write_bundle_header(project_path, board_id, out_path):
    ...
    guard = "MOYBYTE_PROJECT_BUNDLE_" + _c_identifier(manifest.id) + "_H"
    with open(out_path, "w", encoding="utf-8") as fh:
        ...
        fh.write("#define MOYBYTE_PROJECT_ID \"" + manifest.id + "\"\n")
        fh.write("#define MOYBYTE_PROJECT_TITLE \"" + manifest.title + "\"\n")
        fh.write("#define MOYBYTE_PROJECT_BOARD \"" + profile["id"] + "\"\n")
        fh.write("#define MOYBYTE_PROJECT_BUNDLE_SIZE " + str(len(data)) + "\n\n")
        fh.write("static const uint8_t MOYBYTE_PROJECT_BUNDLE[] = {\n")
