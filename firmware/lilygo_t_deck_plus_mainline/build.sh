...
# The shared trees are repo-root now (the fork that owned them is deleted):
#   native/   the shared C usermods        patches/  the IDF/MicroPython patches
# Nothing here may resolve inside a BOARD directory -- that coupling is what
# the deletion removed, and a leftover ${FORK_DIR} pointing at this board made
# both of these silently resolve to paths that do not exist.
SHARED_NATIVE_DIR="${REPO_ROOT}/native"
...
if [ -z "${IDF_DIR:-}" ]; then
  for _cand in \
      ...
      "${REPO_ROOT}/firmware/lilygo_t_deck_plus_mainline/.build/esp-idf"; do
...
mkdir -p "${STAGED_NATIVE}"
{
  ...
  for m in ${SHARED_NATIVE}; do
    if [ ! -d "${SHARED_NATIVE_DIR}/${m}" ]; then
      echo "!! shared native module ${m} not found in ${SHARED_NATIVE_DIR}" >&2
      exit 1
    fi
    cp -r "${SHARED_NATIVE_DIR}/${m}" "${STAGED_NATIVE}/${m}"
