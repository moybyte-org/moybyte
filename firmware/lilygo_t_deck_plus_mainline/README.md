...
`moybyte_shell.RUN_PANEL_SMOKE` is on and there is nothing else to run: the
shared console is not staged into this build yet. The smoke is
**self-terminating** — it paints and returns to the REPL rather than taking the
loop over, because whether mainline's USB-CDC RX survives on this board is one
of the open questions (under the fork build it did not).
...
Moybyte T-Deck (mainline) shell starting
Moybyte panel: init
...
modules/                boot.py, main.py, moybyte_shell.py, tdeck_panel.py
...
| patch | how it is solved here | does stage 1 need it? |
|---|---|---|
| **REPR_C unboxed floats** (#66) | guarded `sed` on `ports/esp32/mpconfigport.h`'s `MICROPY_OBJ_REPR` line, and the build **fails loudly** if the line has changed shape. A sed rather than the fork's context diff, so it survives the line moving between MicroPython releases. Verified in the built tree. | No — but it is free, it changes object layout so it must be settled early, and the whole image was compiled and linked with it |
| **I2C GIL release** (#69) | a small in-place Python edit that brackets `i2c_master_cmd_begin` with `MP_THREAD_GIL_EXIT/ENTER`. Result is byte-identical to the fork's patched file. Exits non-zero if the call site is not found. | No — nothing reads I2C yet. Required from stage 2 (touch/keyboard), because without it the C3's 40–60 ms clock-stretch stalls block the frame loop no matter which thread reads |
| **esp_lcd `tx_color` no-acquire** (#66) | the fork's `.patch` file, applied to the ESP-IDF tree (idempotent; the shared checkout usually has it already from the fork build) | **No.** `moy_lcd.show()` fences on its own completion counter rather than relying on `spi_device_acquire_bus` happening to serialize the bands, so without the patch the flush is merely serialized. The patch is what lets it *overlap* later |
...
1. **Panel.** ← you are here. Mainline boots, ST7789 comes up, a test pattern
   lands. Compiles clean; needs eyes on glass.
2. Touch (GT911, I2C0 @ 0x5D/0x14) — needs the #69 GIL patch to be real.
3. Keyboard (ESP32-C3 @ I2C0 0x55, both modes).
4. SD (`moy_sd` attach on the live host).
5. Audio (ES8311 / `moy_audio`).
6. The frozen shared console. **The staging step must be the same data-driven
   mechanism the other two boards use** — do not reproduce the fork build's 57
   hand-written `cp runtime/*.py` lines. A per-board allowlist is exactly what
   let a board silently miss shared modules.
