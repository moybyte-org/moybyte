# Moybyte device web view (#41 / #22) -- the DEVICE serves its own console over
...
# <canvas> using the MOY64 palette + the cart's spritesheet from /assets. The device
...
def palette_rgb(pal565):
    """The MOY64 palette as 64 [r,g,b] triples, decoded from the device's RGB565 LUT
    so the browser resolves indices to the SAME colours the panel shows. pal565 is
    moy_runtime.PAL565 (canonical little-endian RGB565, NOT the byte-swapped LUT)."""
    out = []
    for c in pal565:
        r = (c >> 11) & 0x1F
        g = (c >> 5) & 0x3F
...
# (/ws), replay the indexed draw commands each pushed frame against the MOY64 palette, and
