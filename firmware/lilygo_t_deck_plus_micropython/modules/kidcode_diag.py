# KidCode on-device diagnostics -- offline log capture for the LilyGO T-Deck.
#
...
# buffer of log lines (near-zero cost) and flush it to /sd/kidcode/diag.log
...
# capture /dev/ttyACM* at boot -> read the "===== KidCode diag dump =====" block.
#
...
DUMP_HEADER = "===== KidCode diag dump (previous session) ====="
...
def logp(tag, msg):
    ...
    print("KidCode ...") diagnostics through diag use it to mean "printed AND
...
# the SD-session wrapper (kidcode_sd.with_sd_live on device) so the mount happens
...
def flush_to_sd(with_sd):
    """Persist the current RAM ring to /sd/kidcode/diag.log via the supplied SD
    session wrapper (device: kidcode_sd.with_sd_live, which mounts the card on the
...
def _read_prev_log_pre_display():
    ...
    (machine.SDCard via kidcode_sd.with_sd). This is the SAFE read path when
    ...
    try:
        return kidcode_sd.with_sd(_read)
    except Exception:
        return None
...
def dump_previous_to_serial():
    ...
    Call this from kidcode_shell.main() BEFORE _init_display() -- that's the
