// KidCode kc_sd: SD card on the SPI host the display already initialized.
//
...
// side (kidcode_sd._NativeSDBlockDev) wraps it for vfs.mount.
...
#include "sdmmc_cmd.h"
#define KC_SD_HAVE_IDF 1
#else
#define KC_SD_HAVE_IDF 0
#endif

#define KC_SD_SECTOR 512
...
static void kc_sd_release(void) {
    if (s_card != NULL) {
        free(s_card);
        s_card = NULL;
    }
...
static void kc_sd_check(esp_err_t err, const char *what) {
    if (err != ESP_OK) {
        kc_sd_release();
        mp_raise_msg_varg(&mp_type_OSError,
                          MP_ERROR_TEXT("kc_sd %s failed: %d"), what, (int)err);
    }
}
#endif
...
static mp_obj_t kc_sd_init(size_t n_args, const mp_obj_t *args) {
#if KC_SD_HAVE_IDF
    ...
    if (s_card == NULL) {
        kc_sd_release();
        mp_raise_msg(&mp_type_MemoryError, MP_ERROR_TEXT("kc_sd: out of memory"));
    }
    err = sdmmc_card_init(&hostcfg, s_card);
    kc_sd_check(err, "card_init");

    if (s_bounce == NULL) {
        s_bounce = (uint8_t *)heap_caps_malloc(KC_SD_SECTOR, MALLOC_CAP_DMA);
        if (s_bounce == NULL) {
            kc_sd_release();
            mp_raise_msg(&mp_type_MemoryError, MP_ERROR_TEXT("kc_sd: no DMA bounce"));
        }
    }
    ...
    (void)n_args;
    (void)args;
    mp_raise_NotImplementedError(MP_ERROR_TEXT("kc_sd needs ESP-IDF"));
#endif
}
static MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(kc_sd_init_obj, 0, 3, kc_sd_init);
...
static void kc_sd_require(void) {
    if (s_card == NULL) {
        mp_raise_msg(&mp_type_OSError, MP_ERROR_TEXT("kc_sd: not mounted"));
    }
}
#endif

// read(start_block, buf, count) -> None. buf must hold count*512 bytes.
static mp_obj_t kc_sd_read(mp_obj_t start_in, mp_obj_t buf_in, mp_obj_t count_in) {
#if KC_SD_HAVE_IDF
    kc_sd_require();
    ...
    if (bi.len < (size_t)count * KC_SD_SECTOR) {
        mp_raise_ValueError(MP_ERROR_TEXT("kc_sd: read buffer too small"));
    }
    uint8_t *dst = (uint8_t *)bi.buf;
    for (uint32_t i = 0; i < count; i++) {
        esp_err_t err = sdmmc_read_sectors(s_card, s_bounce, start + i, 1);
        kc_sd_check(err, "read");
        memcpy(dst + (size_t)i * KC_SD_SECTOR, s_bounce, KC_SD_SECTOR);
    }
...
static MP_DEFINE_CONST_FUN_OBJ_3(kc_sd_read_obj, kc_sd_read);

// write(start_block, buf, count) -> None. buf must hold count*512 bytes.
static mp_obj_t kc_sd_write(mp_obj_t start_in, mp_obj_t buf_in, mp_obj_t count_in) {
#if KC_SD_HAVE_IDF
    kc_sd_require();
    uint32_t start = (uint32_t)mp_obj_get_int(start_in);
    uint32_t count = (uint32_t)mp_obj_get_int(count_in);
    ...
    if (bi.len < (size_t)count * KC_SD_SECTOR) {
        mp_raise_ValueError(MP_ERROR_TEXT("kc_sd: write buffer too small"));
    }
    const uint8_t *src = (const uint8_t *)bi.buf;
    for (uint32_t i = 0; i < count; i++) {
        memcpy(s_bounce, src + (size_t)i * KC_SD_SECTOR, KC_SD_SECTOR);
        esp_err_t err = sdmmc_write_sectors(s_card, s_bounce, start + i, 1);
        kc_sd_check(err, "write");
    }
    return mp_const_none;
#else
    (void)start_in; (void)buf_in; (void)count_in;
...
static MP_DEFINE_CONST_FUN_OBJ_3(kc_sd_write_obj, kc_sd_write);

// sector_count() -> total 512-byte sectors on the card (0 if not mounted).
static mp_obj_t kc_sd_sector_count(void) {
#if KC_SD_HAVE_IDF
...
static MP_DEFINE_CONST_FUN_OBJ_0(kc_sd_sector_count_obj, kc_sd_sector_count);
...
static mp_obj_t kc_sd_deinit(void) {
#if KC_SD_HAVE_IDF
    kc_sd_release();
#endif
    return mp_const_none;
}
static MP_DEFINE_CONST_FUN_OBJ_0(kc_sd_deinit_obj, kc_sd_deinit);

static const mp_rom_map_elem_t kc_sd_globals_table[] = {
    { MP_ROM_QSTR(MP_QSTR___name__),     MP_OBJ_NEW_QSTR(MP_QSTR_kc_sd) },
    { MP_ROM_QSTR(MP_QSTR_init),         MP_ROM_PTR(&kc_sd_init_obj) },
    { MP_ROM_QSTR(MP_QSTR_read),         MP_ROM_PTR(&kc_sd_read_obj) },
    { MP_ROM_QSTR(MP_QSTR_write),        MP_ROM_PTR(&kc_sd_write_obj) },
    { MP_ROM_QSTR(MP_QSTR_sector_count), MP_ROM_PTR(&kc_sd_sector_count_obj) },
    { MP_ROM_QSTR(MP_QSTR_deinit),       MP_ROM_PTR(&kc_sd_deinit_obj) },
    { MP_ROM_QSTR(MP_QSTR_SECTOR_SIZE),  MP_ROM_INT(KC_SD_SECTOR) },
};
static MP_DEFINE_CONST_DICT(kc_sd_globals, kc_sd_globals_table);

const mp_obj_module_t mp_module_kc_sd = {
    .base = { &mp_type_module },
    .globals = (mp_obj_dict_t *)&kc_sd_globals,
};

MP_REGISTER_MODULE(MP_QSTR_kc_sd, mp_module_kc_sd);
