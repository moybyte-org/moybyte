...
mainline-MicroPython build** (ESP-IDF 5.5, native C modules, frozen modules). The
Zero deliberately does **not** need that. It runs **stock MicroPython** + pushed
`.py` files, so the dev loop is `mpremote cp` with no build step.
