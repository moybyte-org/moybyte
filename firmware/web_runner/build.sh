...
# --spec SLIM PLAYER: the spec bundle never reaches the console shell (kiosk
# boots straight into the cart; no Make tile, no shelf visit, no Settings), so
# the shell-only modules freeze as AUTO-GENERATED permissive stubs -- import-
# compatible name-for-name (classes/functions scanned from the real module's
# AST), every attribute/call absorbing to a falsy no-op. The PLAY path stays
# real, including crash display (editors/editor_app/code_layer -- `moy run`'s
# dev loop needs the crash-to-code screen). The full Moybyte dist keeps the
# whole console.
if [ "${SPEC}" = "1" ]; then
  SLIM_STUBS="block_editor_ui map_editor_ui music_editor_ui scene_editor_ui
    cards_layer paint_layer settings_layer appearance_app files_app
    file_widgets writer_app storybook_app sheets_app formula calc_app
    app_shell update_ui system_menu_ui achievements_ui blocks
    editors_block editors_music editors_paint_map editors_scene"
  "${PY}" - "${STAGE_DIR}/modules" ${SLIM_STUBS} <<'PYEOF'
import ast, os, sys
d = sys.argv[1]
HEAD = '''"""AUTO-STUB (--spec slim player, generated by build.sh): console-shell
functionality the cart player never reaches; satisfies imports only."""
...
for name in sys.argv[2:]:
    ...
    for node in ast.parse(src).body:
        if isinstance(node, (ast.ClassDef, ast.FunctionDef)):
            names.append((node.name, isinstance(node, ast.ClassDef)))
        elif isinstance(node, ast.Assign):
            # Constants keep their REAL values -- other (real) modules import
            # geometry numbers from these files (chrome <- settings_layer);
            # anything not literal-evaluable degrades to the absorbing stub.
            try:
                v = repr(ast.literal_eval(node.value))
            except Exception:
                v = "_S"
            for t in node.targets:
                if isinstance(t, ast.Name):
                    names.append((t.id, v))
    out = HEAD
    for n, is_cls in names:
        if n.startswith("__"):
            continue
        if is_cls is True or is_cls is False:
            # Classes AND functions become the one absorbing INSTANCE: class-
            # attribute reads (Layout.MIN_W), construction, and calls all
            # resolve through _Stub.__getattr__/__call__ (MicroPython has no
            # metaclasses, so a stub CLASS couldn't absorb attr reads).
            out += "%s = _S\n" % n
        else:
            out += "%s = %s\n" % (n, is_cls)
    total_saved += len(src) - len(out)
    open(p, "w", encoding="utf-8", newline="\n").write(out)
print("  slim: %d modules stubbed, %.0f KB source dropped"
      % (len(sys.argv) - 2, total_saved / 1024))
PYEOF
fi
