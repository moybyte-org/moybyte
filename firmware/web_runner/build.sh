...
#
#   ./build.sh              # stage + build + assemble dist/
#   ./build.sh --stage-only # just re-stage modules/carts/page (fast dev loop;
#                           # the wasm VM rarely changes)
#
...
PYEOF
  ...
  echo "== building MicroPython webassembly (moybyte variant + moy_lua)"
...
# ---------------------------------------------------------------------------
# 5. Assemble dist/ (static -- host anywhere; python -m http.server works).
# ---------------------------------------------------------------------------
mkdir -p "${DIST_DIR}"
cp "${PORT_DIR}/build-moybyte/micropython.mjs" "${DIST_DIR}/"
cp "${PORT_DIR}/build-moybyte/micropython.wasm" "${DIST_DIR}/"
cp "${STAGE_DIR}/carts.json" "${STAGE_DIR}/modules.json" "${STAGE_DIR}/index.html" "${DIST_DIR}/"
echo "== dist/ ready:"
ls -la "${DIST_DIR}"
