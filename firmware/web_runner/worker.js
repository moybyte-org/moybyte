...
function cartNames() {
    const out = [];
    if (!mp) return out;
    for (const name of mp.FS.readdir(CARTS)) {
        if (name === "." || name === ".." || store.skipName(name)) continue;
        try { if (vfsIsDir(CARTS + "/" + name)) out.push(name); } catch (e) { }
    }
    return out.sort();
}
...
async function importZip(name, buf) {
    ...
    const dir = store.uniqueCartDir((n) => vfsExists(CARTS + "/" + n),
