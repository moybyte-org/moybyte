...
# The boot spine + frame pump, shared with every board (#161 Phase 4/5,
# canonical: runtime/device_boot.py; board.toml stages it like every other
# shared module): boot splash, cart seed+scan, the Lua runtime probe, the OTA
# verdict + rollback confirm, the frame cadence, the idle screen blank
# (IdleBlank) and the serial dev channel (runtime/dev_channel.py). What stays
# here is hardware wiring.
...
def run_desktop(fps_cap=60):
    ...
    comp = P4Compositor()
    gfx = comp.gfx()
    print("Moybyte Guition P4 display up (%dx%d, gfx=%s)"
    ...
    print("Moybyte Guition P4 PPA:", "enabled" if P4SystemCanvas.enable_ppa() else "CPU-only")
    ...
    boot = DeviceBoot(sys_canvas, comp, set_backlight, "Moybyte Guition P4")
    boot.note("starting")
    ...
    try:
        ...
        ws.link = make_link(board="guition_p4",
                       name=ws.system.get("name", "guition_p4"))
        ws.net = ws.link.net
    except Exception as exc:  # noqa: BLE001 -- no radio must never cost a console
        print("Moybyte Guition P4 link unavailable:", exc)
        ws.link = None
    ...
    try:
        ...
        print("Moybyte Guition P4: OTA updater unavailable:", exc)
    ...
    try:
        ...
        print("Moybyte Guition P4: C6 updater unavailable:", exc)
    try:
        import machine
        ws.reboot_hook = machine.reset
    except Exception as exc:  # noqa: BLE001
        print("Moybyte Guition P4: reboot hook unavailable:", exc)
    ...
    try:
        ...
        print("Moybyte Guition P4: web console unavailable:", exc)
    ...
    try:
        ...
        print("Moybyte Guition P4 serial channel unavailable:", exc)
        serial = None
    ...
    _ota = OtaHealth(ws, log=lambda m: print("Moybyte Guition P4 OTA: %s" % m))
    _ota.boot_check()
    print("Moybyte Guition P4 desktop running (Ctrl-C for REPL)")
    ...
    def _poll_inputs(now):
        ...
        outside the NimBLE IRQ) and the GSL3680 pointer. The dev channel and the
        ...
        try:
            ...
            print("Moybyte Guition P4 BLE keyboard poll failed:", exc)
        inp.begin_frame()
    ...
    def _frame_error(exc):
        print("Moybyte Guition P4 frame error:", exc)
        gc.collect()
