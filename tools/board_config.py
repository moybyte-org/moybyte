...
def native_denials(board_dir):
    ...
    nat = load(board_dir).get("native", {}).get("shared", {})
    return {e["module"]: e for e in nat.get("deny", [])}
...
def native_modules(board_dir, root=ROOT):
    ...
    denies it with a reason. Boards with no [native] section return [] (the
    web runner's emscripten staging is structurally different and stays in its
    build.sh).
    """
    cfg = load(board_dir)
    nat = cfg.get("native", {}).get("shared", {})
    if not nat:
        return []
...
def stage_native(board_dir, root=ROOT, quiet=False):
    ...
    cfg = load(board_dir)
    mods = native_modules(board_dir, root)
    if not mods:
        return []
    src_dir = root / cfg["native"]["shared"].get("source", "native")
    ...
    for m in mods:
        shutil.copytree(src_dir / m, dest / m,
