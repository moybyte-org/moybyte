"""Host-testable unit tests for the device-only kidcode_diag module's PURE logic:
the bounded ring buffer, the line/perf formatting, and the dump markers. The SD
persistence + boot-dump paths touch hardware (machine.SDCard / kidcode_sd) and are
covered by the grep tests in test_micropython_spike.py instead.

kidcode_diag lives in the firmware modules/ tree (device-only) but its top-level
...
DIAG_SRC = Path("firmware/lilygo_t_deck_plus_micropython/modules/kidcode_diag.py")
...
def diag():
    ...
    spec = importlib.util.spec_from_file_location("kidcode_diag_test", DIAG_SRC)
...
def test_log_persists_and_echoes_live(diag, capsys):
    ...
    assert "KidCode" in out and "audio I2S ready" in out         # echoed live
...
def test_dump_markers_are_stable(diag):
    ...
    assert diag.DUMP_HEADER == "===== KidCode diag dump (previous session) ====="
...
def test_flush_to_sd_writes_whole_ring_via_wrapper(diag):
    ...
    # Stand in for kidcode_sd.with_sd_live: just runs fn() (host has no SD).
    def fake_with_sd(fn):
