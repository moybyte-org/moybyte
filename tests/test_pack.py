...
def test_pack_project_creates_kc8_bundle(tmp_path):
    ...
    with zipfile.ZipFile(out) as archive:
        ...
        assert "kidcode_bundle.json" in names
        assert "manifest.json" in names
        assert "main.py" in names
        metadata = json.loads(archive.read("kidcode_bundle.json").decode("utf-8"))
        assert metadata["schema"] == "kidcode.bundle.v1"
...
def test_pack_project_excludes_generated_by_default(tmp_path):
    ...
    generated = tmp_path / "generated_game.kcproj" / "generated"
    generated.mkdir()
