...
from moybyte.errors import MoybyteRuntimeError
from moybyte_sim.headless_backend import HeadlessSimulator


def test_music_player_stub_logs_audio_call():
    sim = HeadlessSimulator("examples/music_player_stub.moyproj")
...
def test_radio_pong_stub_sends_join_message():
    sim = HeadlessSimulator("examples/radio_pong_stub.moyproj")
    context = sim.run(frames=3)
...
def test_radio_permission_failure_is_friendly(tmp_path):
    project = tmp_path / "no_radio.moyproj"
    project.mkdir()
