...
def manifest():
    assert os.path.isfile(MANIFEST), (
        "no %s -- run `make vendor-p8-import`" % os.path.relpath(MANIFEST, ROOT))
...
def test_vendored_file_matches_the_manifest(manifest):
    ...
    for rel, want in sorted(manifest["files"].items()):
        ...
        assert _sha256(path) == want, (
            ...
            "This file is a COPY of moy-spec's p8_import.py and must not be "
            "edited here: fix it upstream and re-run `make vendor-p8-import`. "
            "If you did re-vendor, the manifest is stale -- run it again to "
            "re-stamp." % rel)
...
def test_no_drift_from_the_pinned_upstream(manifest):
    ...
    repo), and skipped when the checkout is at a different commit.
    """
    spec = _spec_checkout()
    if not spec:
