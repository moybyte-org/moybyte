...
`conformance/parity.py --ref <moybyte>` replays every scene through
`runtime/canvas.py`, and `tools/p4_conformance.py` runs the carts on a board over
...
- `tests/test_spec_conformance.py` (here) — replays `traces/` through
  `runtime/canvas.py` and hashes the framebuffer against `hashes.json`. The
  **host** rasterizer, on every `make test`, with no spec checkout.
- `tests/test_device_canvas_parity.py` — host vs device, so the device inherits
  the above through it.
