# Moybyte Console — Comprehensive Design Doc v0.3

**Project:** Kids Edu Slate / Laptop → Moybyte Console / Moybyte OS  
...
> **Moybyte Console is a child’s first programmable creative computer: a chunky keyboard-first ESP32-class game-making console where kids can draw sprites, write real code, press Run, play instantly, share locally, play multiplayer, and optionally receive safe AI help.**
...
Sandboxed project runtimes:
    ...
    - Moybyte game/app APIs shared across runtimes
```
...
> Implement desktop as a theme/mode on top of the same Moybyte shell, not as a separate OS.
...
- expose safe Moybyte APIs for graphics, input, sound, storage, radio, and hardware
...
- Best architecture: make the Moybyte API language-neutral so Lua and MicroPython can share the same graphics/input/hardware services.
...
1. v0: Moybyte action/cards model + one embedded scripting runtime.
...
- Moybyte canvas = user app/game output surface
...
- **Moybyte Kernel** as the internal services layer
...
**Moybyte Console is a keyboard-first creative coding console for kids: draw sprites, write simple real code, press Run, play instantly, share locally, and learn computing through making.**
...
Project runtimes
  ...
  - Moybyte-8 game runtime
```

### 6.2 Moybyte Kernel responsibilities

The Moybyte Kernel is an internal services layer, not a real Unix kernel.
...
### 7.1 Moybyte-8 fantasy console

Moybyte-8 is the creative runtime.
...
> Design the Moybyte API so either language can bind to it. Pick one runtime for the first shippable loop.

Recommended first implementation path:

- build Moybyte-8 runtime in C/C++ over ESP-IDF
- implement Cards/Blocks as data that calls Moybyte APIs
...
- still hide raw MicroPython complexity behind Moybyte APIs
...
- same Moybyte runtime/core where possible
- fake input/events
...
### 16.1 Scope direction

*(The 2026-05 prototype hardware targets that stood here are
superseded and no longer tracked in this repo. The conclusion this section
reached — keep v1 scoped so it does not drift into cheap-tablet/Chromebook
territory — is carried forward by the current plan's tier lineup.)*
...
1. **Name internally:** Moybyte Console hardware, Moybyte OS shell, Moybyte-8 runtime, Moybyte Studio IDE.
2. **Prototype hardware:** LilyGO T-Deck first.
3. **Firmware base:** ESP-IDF + FreeRTOS.
4. **GUI:** LVGL.
5. **Runtime output:** controlled Moybyte canvas, not raw LVGL.
