# Moybyte Console - MicroPython-First Design Doc v0.1

**Project:** Moybyte Console / Moybyte OS  
**Document version:** 0.1  
**Date:** 2026-06-18  
**Status:** Parallel architecture branch to `moybyte_Console_Design_Doc_v0_3.md`  
**Purpose:** Explore the product and firmware direction where Moybyte is built around MicroPython from day one, instead of using Lua as the primary device runtime.

---

## 0. Executive conclusion

This document describes a more Python-forward version of Moybyte:

> Moybyte Console is a MicroPython-first creative coding console where the shell, apps, editor, and child projects are all built around Python-style code, with native modules used only where performance, hardware access, or safety require them.
...
ESP-IDF + FreeRTOS base
  ...
  |
  Moybyte OS shell written mostly in MicroPython
  |
  Moybyte framework modules
  |
  Moybyte Studio apps
  |
...
> MicroPython may be good enough if Moybyte keeps graphics, input polling, audio, storage safety, and any future 3D helpers behind native-backed APIs, while child code stays at the game/app logic level.
...
MicroPython runtime
LVGL bindings
MicroPython Moybyte shell
MicroPython Moybyte apps
...
```text
Moybyte OS is written in Python.
...
Moybyte does not need to copy MicroPythonOS or port it directly to the LilyGO T-Deck Plus, but it can learn from the pattern:
...
Moybyte still needs to be a controlled creative console, not a generic MicroPython handheld.
...
- LVGL bindings
- frozen Moybyte framework modules
...
Child projects should not manipulate arbitrary LVGL widgets in beginner mode. They draw to a Moybyte canvas API.

### 4.4 Moybyte framework

Moybyte provides frozen/default modules:
...
There is one MicroPython VM. The Moybyte shell is running inside it. Project execution is a controlled load-and-call operation.
...
```json
{
  "format": "moybyte-project-v1",
  "title": "Tiny Runner",
  "runtime": "moybyte-micropython",
  "entry": "main.py",
...
env = moybyte_project_environment(project_manifest)
exec(source, env)
```
...
1. Soft enforcement: editor templates and lessons only expose `moybyte`.
...
The `moybyte` API checks permissions before performing service actions.
...
- periodic yield points in Moybyte APIs
...
- starts frame loop
- maps input into Moybyte buttons
...
> Can a MicroPython/LVGL Moybyte shell on T-Deck Plus run Tiny Runner with acceptable input feel, frame rate, restart behavior, and recovery from bad code?

### 12.2 Required demo

Must show:

1. boots to simple Moybyte launcher
2. keyboard/trackball diagnostic screen
3. Tiny Runner project loaded from file
4. `from moybyte import *` project API
...
### Task group B - Moybyte module

- create `moybyte` MicroPython API skeleton
...
> Moybyte starts with kid-friendly projects and grows into real MicroPython.
...
1. Is MicroPythonOS worth borrowing from, or should Moybyte use only its architectural ideas?
...
The next concrete step is not a full OS. It is a Tiny Runner spike with a frozen/default `moybyte` module and measured recovery behavior.
