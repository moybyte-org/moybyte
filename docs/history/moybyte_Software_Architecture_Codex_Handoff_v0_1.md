# Moybyte Software Architecture — Codex Implementation Handoff v0.1

**Project:** Kids Edu Slate / Laptop → Moybyte Console pivot  
**Document:** Software architecture and implementation plan  
**Version:** 0.1  
**Date:** 2026-06-16  
**Intended reader:** Codex / engineering agent / future developer  
**Primary goal:** Build a PC-first Moybyte prototype with a real edit-run-test loop, then prepare the path to ESP32 firmware.
...
```text
You are implementing Moybyte, a PC-first simulator and SDK for a future ESP32-based kids' creative coding console.

Read this architecture document completely before coding.

Build the repo in small, testable steps. Prioritize:
1. A working Python package named `moybyte` that exposes the child-facing API.
2. A CLI named `moybyte` that can create, validate, compile, and run projects.
3. A desktop simulator that runs `.moyproj` projects with a scaled 128x128 fantasy-console canvas.
...
- `moybyte run examples/tiny_runner.moyproj` opens a simulator window or runs headless in CI.
- The player sprite moves from keyboard input.
- `moybyte compile examples/blocks_demo.moyproj` generates valid Python from blocks.
- `pytest` passes.
...
Moybyte is not only a fantasy console and not only a MicroPython terminal.
...
The fantasy console is the friendly creative shell. The real programming model is a small, stable Moybyte API that can run in a PC simulator first and later on ESP32/MicroPython firmware.
...
```text
1. Moybyte SDK / Runtime API
   The Python-facing API used by kid projects.

2. Moybyte Simulator / Tooling
...
User code should import from `moybyte`, not directly from hardware libraries:
...
Blocks are a UI/view over a constrained Moybyte program model. They should generate Python/Moybyte code.

```text
blocks.json → Moybyte AST → generated main.py → runtime
```
...
```python
import os
from moybyte.system import audio_service
```
...
Moybyte has two different execution worlds:
...
The public `moybyte` package should be deliberately simple: plain classes, functions, lists, dictionaries, small constants, and backend injection points. The PC simulator can inject desktop services behind that API. Later, the firmware or MicroPython port can provide the same API names with native services.
...
Use boring, easy Python dependencies for PC tooling and simulator code. Do not leak these dependencies into the portable `moybyte` runtime layer or generated user project code.
...
- `pygame` gives the fastest desktop simulator path, but belongs in `moybyte_sim`.
...
- `typer` and `rich` make the CLI pleasant, but belong in `moybyte_cli`.
- Keep dependencies minimal; this should run easily on Linux/macOS/Windows.
- Keep `moybyte/` portable enough that it can later be reimplemented or shimmed on MicroPython without rewriting kid projects.

If pygame causes install problems, keep the headless backend working so CI/tests still run.

---

## 6. Project format: `.moyproj`
...
|---|---:|---|
| `schema` | yes | Must be `moybyte.project.v1` for v0 |
...
`from moybyte import *` should expose only safe kid-facing names.
...
```bash
moybyte screenshot examples/tiny_runner.moyproj --frames 10 --out /tmp/tiny_runner.png
```
...
CLI command name: `moybyte`.

### 13.1 Commands

```bash
moybyte new my_game --kind game
moybyte validate examples/tiny_runner.moyproj
...
moybyte pack examples/tiny_runner.moyproj --out tiny_runner.kc8
moybyte doctor
```
...
```bash
tools/check_micropython_subset.py examples/tiny_runner.moyproj/main.py
```
...
```text
no external imports except moybyte/math/random/time/os subset
no pathlib
...
Moybyte native modules/services
...
import os
from moybyte.system import audio_service
from moybyte.hardware import i2c
```
...
```bash
moybyte run examples/music_player_stub.moyproj --headless --frames 30
...
A developer can run `moybyte run examples/tiny_runner.moyproj`.
...
7. Implement `moybyte run --headless`.
8. Implement `PygameBackend`.
9. Create `tiny_runner.moyproj`.
10. Implement block schema and compiler.
11. Create `blocks_demo.moyproj`.
12. Implement fake audio service and `music_player_stub.moyproj`.
13. Implement fake radio service and `radio_pong_stub.moyproj`.
14. Add friendly error conversion.
15. Write docs for project format and Moybyte API.
...
```text
Build Moybyte as a PC-testable creative coding runtime first.
