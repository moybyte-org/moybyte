# Moybyte Native Graphics Core — Stage 3 Plan (production compositor)
...
`modules/moy_compositor.py` runs on the T-Deck via the `RUN_COMPOSITOR_SMOKE`
...
- **Fix (validated):** flush the whole screen in **one** `tx_color` from a
  ...
  reason the 128×128 `moy_canvas` blit was always clean. Small dirty rects still
...
## Connect-the-ends (2026-06-23): a .moy runs on device

`modules/moy_runtime.py` runs an actual v0.4 cartridge on the T-Deck: a
...
loading real `.moy` files from SD is the follow-on). Behind `RUN_KIDCART`.
...
canvas backend differs (`runtime/canvas.py` ↔ `moy_runtime.DeviceCanvas`).
...
ESP32-S3 / FreeRTOS
  └─ Native KidKernel (C)
       ├─ moy_gfx       : VM-neutral pixel kernel (fill / fill_rect / blit / pack)
       │                 operates on RGB565 buffers; no MicroPython/LVGL/Lua deps
       └─ lcd_bus      : existing SPI DMA panel path (tx_param / tx_color)
            └─ moy_compositor (Python today): owns the framebuffer + dirty regions,
               draws via moy_gfx, flushes only dirty bands over lcd_bus DMA
                 └─ userland VM (MicroPython now; Lua later) calls the same
                    language-neutral surface: kid.clear/rect/spr/text → moy_gfx
```

The strategic point (v0.4): **the pixel kernel is C and VM-neutral.** `moy_gfx` has
...
- `native/moy_gfx/` C module: `fill`, `fill_rect`, `blit565` (with transparent
...
- `moy_compositor.py` rewritten: owns an RGB565 framebuffer + a **dirty-region
  tracker**, draws through `moy_gfx` (falls back to `framebuf`/pure-Python when the
...
- Native font blitter in `moy_gfx` (8×12 / 6×8 / 12×16, per TulipCC) to replace
...
- Lua bindings to `moy_gfx` once the userland decision lands.
...
```
moy_gfx.fill(buf, npix, color565)
moy_gfx.fill_rect(buf, stride_px, x, y, w, h, color565)
moy_gfx.blit565(dst, dst_w, dst_h, dx, dy, src, src_w, src_h, key)   # key=-1: opaque
moy_gfx.pack_strip(fb, fb_w, x, y, w, rows, dst)                      # fb window -> dst (contiguous)
```
...
for each (y, rows) band of the dirty box (<= strip height):
    moy_gfx.pack_strip(fb, fb_w, x, y, w, rows, strip_dma)   # C
...
`native/moy_gfx/` mirrors `native/moy_alloc/` (`modmoy_gfx.c` + `micropython.cmake`).
`build.sh` stages it into `ext_mod/moy_gfx` and injects
`include(.../moy_gfx/micropython.cmake)` after the `moy_alloc` include — same
re-stage-every-build pattern (the upstream `ext_mod` is wiped on re-clone).

## Migration from the Stage 2 spike

- `moy_compositor.py` (Stage 2 strip blitter + benchmark helper) is rewritten into
  the dirty-rect compositor; `plan_strips()` and the host `make_compositor(None)`
  guard are kept. The fullscreen benchmark in `moybyte_shell.py`
  (`RUN_FULLSCREEN_BENCH`, default off) stays as a regression/A-B tool.
- `moy_canvas.py` (Stage 1 128×128 blitter) stays for the game path.
- The renderer/desktop will target `moy_compositor` at full screen instead of the
  128×128 canvas once the desktop shell is built (separate task).

## Device-test checklist (when validating v1 on hardware)

1. Build with `MOYBYTE_SKIP_VFS_BOOT=1`; confirm `moy_gfx` links (no build error).
...
- **moy_gfx compiles but is untested on hardware** (no device access at authoring
  time). The ops are simple, bounds-clamped, and mirror the proven moy_alloc module
