...
class BandedCompositor:
    ...
    def bounce_stats(self):
        """The whole `moy_flush` pump tuple, all nine fields:

            (pump_us, idle_us, idle_n, feed_us, bands,
             blocked_us, timeouts, errs, stop_fails)
        ...
        timeouts, errs and stop_fails must all stay 0. `moy_flush` cannot
        RAISE a queue error hit during a drain -- a drain must not throw into
        the frame loop -- so these counters are the only place such a failure
        is visible at all: a flush that is quietly failing looks healthy from
        every other angle. stop_fails is deinit's: the feeder never
        acknowledged, so the bounce slots were LEFT allocated rather than
        freed under a live ISR, and the transport was left up.
        """
        if not self._async:
        ...
        return (st[0], st[1], st[2], st[3], st[4], st[5], st[6],
                st[7], st[8])
