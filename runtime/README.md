...
| `editors.py` (+ `editors_base/_code/_sheet/_paint_map/_block/_music.py`) | **(shared, staged to device)** the editor cores, split per editor with `editors.py` as the re-exporting umbrella (`from editors import X` unchanged): `editors_base` = `UndoStack`/`KeyEdge`/`UndoRedoMixin`; `CodeEditor`; `_SheetSprite`/`SpriteSheet` (8×8 tiles + `__gfx__` hex)/`IconSheet`/`TileMap` (`w×h` tile-id grid over a sheet + `map.moymap` hex, `mget`/`mset`, #32); `PaintEditor`+`MapEditor`; `BlockRow`+`BlockEditor`; `MusicEditor` |
...
| `moy_journal.py` | **(shared, staged)** the per-project undo/redo journal (#7 Stage 7: `journal.jsonl` append-only + full-file snapshots + atomic cursor + rotation; the Stage 8 graduation rider) |
