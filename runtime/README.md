...
| `editors.py` (+ `editors_base/_code/_sheet/_paint_map/_block/_music/_scene.py`) | **(shared, staged to device)** the editor cores, split per editor with `editors.py` as the re-exporting umbrella (`from editors import X` unchanged): `editors_base` = `KeyEdge` (the typed-key edge, with the guarded seed a modal prompt opens on) + `TextEntry`/`text_key` (the ONE on-screen text-entry ladder: Backspace/Delete trim, Enter commits, Esc cancels, printable ASCII appends under a cap; every typed field on the console, from the block keypads to the wifi password, is one of these); `CodeEditor`; `_SheetSprite`/`SpriteSheet` (8×8 tiles + `__gfx__` hex)/`IconSheet`/`TileMap` (`w×h` tile-id grid over a sheet + `map.moymap` hex, `mget`/`mset`, #32); `PaintEditor`+`MapEditor`; `BlockRow`+`BlockEditor`; `MusicEditor`; `SceneEditor` (#85 Stage 2: placed-actor rows + place/select/move/z-order/props over full-snapshot undo) |
...
| `wm_windowed.py` | **(shared, host/P4 only — NOT staged to the S3)** `WindowedWM` — the Picotron-style windowed desktop (#73/#58, spec §3's big-screen tier): the Library is the full-screen launch surface; PLAY/CHANGE leaves it for the wallpaper desktop, where every pushed process is a window with a WM title strip (min/max/close), draggable + grip-resizable and mirrored as a taskbar chip in the desktop's OS bar (restore/minimize/raise). `ws.windowed_chrome` strips OS chrome (right zone, dock) from in-window bars so the desktop bar is the one taskbar; a running cart composites integer-scaled + centered, so the editor stays visible beside a playtest. Per-window layout contexts re-run the #39 responsive layouts at each window's size. On the wasm head it is simply another raster tier -- real layer buffers, composites via `blit_game`/`blit_cover`, no special cases left (the recorder they served went in the 2026-08 sunset). `simulate_desktop.py --size 1024x600 --windowed` |
| `layers.py` | **(shared, staged)** the `Layer` protocol + `_LegacyLayer` (the draw-only overlays' layer: splash/toast/notice/cursor) + the thin object-surface adapters (blocks/map/music/update/sysmenu/about/achievements/perf) |
...
| `cards_layer.py` | **(shared, staged)** `CardsLayer` — the "Make it mine" config-card editor (#3/#15): card draw + layout + scroll (msel/mtop) + taps; cart `config`/`apply`/`adjust` stay on `ws`. A malformed `edit` field def (#94) degrades to one inline "!" card via `_validate_field` instead of taking the whole tab down. The header **INFO button** opens the **CART INFO dialog** (#94) that edits the manifest's title/author through `ws.project.commit_manifest` (`moy_carts.save_manifest_meta`); `permissions` stays read-only. CART INFO and NEW SCRIPT are ONE `_Prompt` state machine (fields of `TextEntry` over a shared guarded key edge, opened through `widgets.arm_prompt`), parameterised by its fields and by what OK does. The last row of the card column is the **ADVANCED row** (docs/text_editing_2026-09.md step 5): it lists the project's own folder over `moy_carts.PROJECT_KIND` and ROUTES -- any of the cart's SCRIPTS to the Code tab (#89: its whole load order, not just main), an asset to its own tab, a text file through `ws.open_project_file`, which opens it in the editor handle and returns HERE, so the loader re-reads the folder. A cart whose manifest will not parse is recovered by `moy_carts.load` rather than dropped, and this tab is where it says so. The row's last entry is **+ NEW SCRIPT** (#89), the ONE door that adds a file to a cart, offered only where a second script would actually run (`moy_carts.multi_script`) |
...
| `code_layer.py` | **(shared, staged)** `CodeLayer` — the full-screen code editor (#24/#39): draw + touch/keyboard editing + the MicroPython-safe syntax highlighter; `ws.editor`/`save_code`/`run_code` stay on `ws`. The tab holds a FILE, not a cart (#89): on a cart of several scripts (SPEC.md §4's `sources` — a PICO-8 port is `p8.lua`, `main.lua` and one file per PICO-8 tab) a FILE chip left of the TLS toggle names the open one and opens the switcher popup; at one source the chip does not exist at all. Which file is `ws.code_file` / `ws.open_code_file`, a switch is an exit path for the file being left (the tab-switch discipline, #111/#154), and there is no CREATE here — that door is the Config tab's ADVANCED row |
| `text_modes.py` | **(shared, staged)** the editor MODE table (docs/text_editing_2026-09.md step 2): a file NAME -> `md`/`code`/`json`/`text`, each carrying `wrap`, the highlighter `lang` and the SOFT-save `gate` (#154's split, generalized -- a hard exit always writes and keeps the badge). Extensions are asked of `moy_carts.FILE_KINDS`/`DOC_EXT`, never spelled. Asked by `code_layer` (the Code tab's language), `editor_handle` (the open document's wrap/highlight/gate) and `files_app`'s router |
| `text_console.py` | **(shared, staged)** `TextConsole` -- `ws.script_console`, the surface a SCRIPT runs on (docs/text_editing_2026-09.md step 6): a bounded, preallocated scrollback RING that `print()` appends to (wrapped as it is written), a prompt line over the shared `CodeEditor` with the Code tab's own symbol palette, drag/scroll, and the two verbs a script talks to them with. The SHELL keeps the pen: `Workstation.run_script` makes `TextConsole.draw` the run's `_draw` and wraps its `_update`, so a script cannot paint over the prompt it is reading from and a raise lands in the scrollback instead of on the crash panel. `input()` POLLS -- None until a line is entered -- because the loop cannot block; a generator `_update` is stepped once per frame, which is the linear way to spell the same wait. `LUA_PRELUDE` binds `print`/`input` for the Lua tier, where libmoy owns the NAME `print`. The terminal (#115) inherits all of it through `on_line` |
| `editor_handle.py` | **(shared, staged)** `EditorHandle` — the console's text editor exposed to a CART (docs/text_editing_2026-09.md step 3, #112): `open_editor(name)` on the `files` grant hands one back, identified by `(kind, name)` through the Files role. It owns the `CodeEditor` buffer, the op-history undo, the clipboard, the `text_modes` wrap/highlight/save gate and the Markdown rendering (headings, tappable checkboxes, `[[note]]` links, `![[drawing]]` embeds) behind a per-line layout memo. Built by `Player._open_cart_editor`, pinned per run and hard-flushed by `release_world` (#154). KEYS reach it from the shell — `Workstation.cart_editor_focus` flips `_set_text_mode`, which is also what seeds away the byte that took the keyboard (`b2ff7de`) — while TAPS are forwarded by the cart, which is the only side that knows where it drew the editor. Lua gets the same verbs through `lua_ext`'s handle prelude |
...
| `widgets.py` | **(shared, staged)** self-contained support classes: `Pointer` (cursor), `Achievements` (#21 tracker + catalog), `Popup` (dropdown #52), `Pmem` (cart RAM), `Actor`/`Scenes` (#85 placed-actor scenes — the `scene()`/`load_scene()` data model), `ConfirmTap` (the two-tap guard every destructive button shares: picker DELETE, Storybook TEAR OUT, Paint's NEW and size swap), `arm_prompt` (the input-edge neutraliser every modal prompt opens with, #29), `_SilentAudio`, `_Blit`, and `_in`, the ONE rect hit-test (`ui.rect_in`), imported directly by every surface |
...
| `web_console.py` | **(shared, staged)** `WebConsole` — `ws.web`, the first Workstation collaborator (#209): the pairing pin (minted once, lazily), the paired url, the park/stop/unpark of the glass and the Settings row's label + one toggle funnel. `ws.webhost` stays a flat Workstation attribute; the old `ws.park_web_console`/`toggle_webhost`/`web_pin`/… names survive as fixed-signature forwards |
| `system_store.py` | **(shared, staged)** `StoreHandle` + `SystemStore` — `ws.prefs`, the owner of the `system.json` dict (#209 landing B): the settings `ws.system` is a plain ALIAS of (`load()` mutates it IN PLACE, so every raw reader/writer in the shell stays honest with no migration), the ONE persist funnel behind every Settings toggle, favorites/recents, the crash guard and the web pin, plus the achievements list's two store halves. `StoreHandle` is the (store, root, `can_manage`, `_with_sd`) guard 4-tuple as an object, read THROUGH `ws` per call so the late service injection cannot become a wiring-order trap. What the settings MEAN (`load_system`'s apply cascade) stays kernel |
| `cover_cache.py` | **(shared, staged)** `CoverCache` + `_CoverImage`/`_CoverJob` — `ws.covers`, the Library shelf's cover + icon pipeline (#209 landing C): the bounded LRU of card bitmaps and parsed RLE runs, the per-frame TIME budget (`begin_frame` / `take_deferred`, the frame loop's only two touches), the idle prefetch + prebuild, the cart desktop-icon cache, and the #186 off-heap frees whose jobs-before-payloads order is ONE body (`_drop_payloads`, shared by the store-rescan `invalidate_all` and the RAM-tight tier's `diet_release`). `covers.gen` is the shelf's repaint key and has no `ws` mirror — launcher_layer and wm_windowed read it directly; both grids take `covers.cover_for` as a bound method. No forwards on `Workstation` |
| `cart_manager.py` | **(shared, staged)** `CartManager` — `ws.carts`, the shelf's ROSTER (#209 landing C): the scanned cart list `carts.all` and everything that changes it — `apply` (the ONE re-sync body: covers invalidated BEFORE the re-slim, then both grids), `rescan` (the browser sync's board half), `new`/`dup`/`delete` over the shared `StoreHandle`, the #66 live-set diet (`slim`/`rehydrate`/`reslim`) and the #105 favorites + recents. `carts.all` has no `ws` mirror — a re-scan REBINDS the list, so an alias could not stay honest — and the launcher takes `carts.is_favorite` as a bound method. What a shelf CARD looks like (`_launcher_items`/`_picker_items`/`_real_selected`, the search filter) stays kernel: it is keyed on `can_manage`, the WM world and the app registry, not on the store. One forward, `ws.rescan_carts`, for `moy_webhost`'s construction-time `on_sync` lambda |
| `appearance.py` | **(shared, staged)** `Appearance` — `ws.look`, every verb that changes how the desktop LOOKS (#209 landing D): panel theme + dark/light variant, the widget skin (the one `skin.use` call in the tree), the system font scale and its degradation rule, the wallpaper CHOICE and the top-bar `IconSheet`. `select_wallpaper` lives here as a COORDINATOR — it is the verb that crosses four owners at once (`carts` rehydrate/re-slim, the `wallpaper` component's compile, `prefs` persist, `ach` note) and belonged to none of them. Two things stay on the kernel and say why: `ws.theme_colors`, the token dict ~70 surface sites read per draw (rebound on every swap — the shelf's cache keys fold its `id()`), and `_bar_img_cache`, which backs the shared draw toolkit. Not `appearance_app.py`, which is the picker a kid taps and reaches these through `ctx.theme` / `ctx.wallpaper`. No forwards on `Workstation` |
| `history_router.py` | **(shared, staged)** `HistoryRouter` — `ws.history`, the #111 undo ROUTER (#209 landing E): the bar UNDO/REDO pair over BOTH undo mechanisms — the active Editor tab's in-RAM `op_history.History` first (resolved through `Project._HISTORY_TABS`), then the durable journal walk, SCOPED to that tab's own file(s) so one tab's undo never reverts another's newest commit — plus the code tab's typing burst (open on the first edit, closed into ONE net-diff op on the autosave debounce / an Enter / an undo press), the post-walk workspace reload (fresh cart, rebuilt editor, caret kept) and the Stage-7 idle autosave (#154: EVERY tab, routed through `EditorApp.save_current`, not just code) `frame()` ticks BEFORE the redraw gate. Takes the shared `StoreHandle`. Two warm entries and neither is a forward: `history.idle_tick()` per frame and `history.bar_undo_bits()` per bar-strip cache key — the latter is RAM-ONLY on purpose, because keying the paint path on the journal would cost a disk read per frame. The Desk Lab apps keep their OWN `History` and their own `files/.history/` sidecars; this resolves the active EDITOR surface only. No forwards on `Workstation` |
| `web_console_ui.py` | **(shared, staged)** `WebConsoleUI` — the WEB CONSOLE connection screen (#197), held as `ws.web.ui`: the surface the glass PARKS on while wasm mode is on. The QR of the paired url (`http://<ip>/?pin=NNNN`), SHOW ADDRESS, TURN OFF. Wasm mode is a SWITCH, not a session: `WebConsole.park`/`unpark` + the `parked` flag `go_home` re-parks on |
| `moy_qr.py` | **(shared, staged)** a dependency-free QR encoder — byte mode, EC level L, versions 1-4, one fixed mask. Written rather than vendored because there is no library on a board and the pairing pin is not a constant anything could be baked with; `tests/test_moy_qr.py` pins it against a third-party encoder |
...
| `moy_carts.py` | **(shared, staged to device)** the `.moy` store — scan/load/save_config/save_code (which SCRIPT is its `name`, #89)/save_sprites/save_sounds/save_map/save_scene/save_manifest_meta (#94: title/author)/cart_sources + source_text + add_source (SPEC.md §4's load order, #89)/create/duplicate/delete + the known-WiFi credential store (load_wifi/remember_wifi/forget_wifi → `wifi.json`, #38) (dict carts; `map.moymap` tilemap blob #32; `scenes/*.moyscene` actor tables #85; only `json`+`os`). Re-exports the three extracted leaves below under their pre-split names |
| `moy_fs.py` | **(shared, staged)** crash-safe file primitives (`_write_atomic`'s marker + stamped-`.bak` + publish, and `_read_recover`, whose per-store `.publish` marker is what keeps a littlefs boot scan at one extra read rather than one per file) shared by the store and the journal |
| `moy_image.py` | **(shared, staged)** the portable `moyimg-v1` codec — ONE format (deflated MOY64 indices, window pinned by `MOYIMG_WBITS`), written through `deflate` on a board and `zlib` on the host + the #66/#86 cover-thumb sidecar cache |
...
| `op_history.py` | **(shared, staged)** the #111 universal op-history core: `History`/`OpCodec` (invert-preferred, replay-from-keyframe fallback, segment cap, optional `max_undo` ring) + the shared `TextEditCodec`/`text_diff_op` typing-burst codec. Every Editor tab + the cart editor handle record fine-grained ops into one of these; the bar UNDO/REDO walks it before the journal |
...
| `app_shell.py` | **(shared, staged)** the Desk Lab apps' shared list shell (#78): `ListShellLayout` (chrome-inset frame + notebook-list geometry/row_rect) + `ListShellApp` (is_app identity gate, store readiness, guarded blob load, persist status contract, list scroll/nav, and the row list's TOUCH model — one `ui.ScrollRegion`+`DragTap`, row-snapped like the Settings rows) — the base Files and Storybook derive from |
...
- `system_carts/` — `moy_night.moy` (the default: a STATIC brand-colorway night
  ...
  (`runtime/editor_handle.py`), holding no text of its own. Each note is a named
  `files/docs/<name>.md` user file (#108) — plain Markdown, UTF-8, LF, no
  envelope, so the card opens in any editor on a PC. It claims the Files
  router's text door with the `editor` marker permission, so a `.md` (or a
  `.json`/`.txt`) opened in Files runs it on that document.
