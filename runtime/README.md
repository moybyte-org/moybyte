...
| `web_console_ui.py` | **(shared, staged)** `WebConsoleUI` — the WEB CONSOLE connection screen (#197), held as `ws.web.ui`: the surface the glass PARKS on while wasm mode is on. The QR of the paired url (`http://<ip>:8080/?pin=NNNN`), SHOW ADDRESS, TURN OFF. Wasm mode is a SWITCH, not a session: `WebConsole.park`/`unpark` + the `parked` flag `go_home` re-parks on |
