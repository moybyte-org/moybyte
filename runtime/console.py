...
# switchers HOME/EDIT/PAINT/MAP/BLOCKS shift one stride right so the bar stays a single
# uninterrupted row. The hamburger is drawn via the _glyph BITMAP (not the themeable
# IconSheet) so an already-themed device -- whose saved system_icons.kgfx has no art for
# a new slot -- never shows a blank tile here. Right cluster (clock at x=224..) is far
# clear of the rightmost switcher (blocks ends at x=108).
_SYSMENU_BTN = (2, _BAR_Y, _BAR_ICON, _BAR_ICON)                 # ≡ dropdown toggle
...
class Workstation:
    ...
    def handle_pointer(self):
        ...
        # either is up it's MODAL: a tap is consumed here (a row taps move+select, a tap
        # OUTSIDE dismisses) and never falls through to the screen underneath.
        if click and self._about:
