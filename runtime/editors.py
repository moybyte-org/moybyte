...
consumer and test keeps `from editors import X` / `editors.X`:
  ...
  editors_base      -- KeyEdge + TextEntry/text_key (the input leaf: the
                       typed-key edge and the one on-screen text-entry ladder)
...
try:
    from editors_base import (KeyEdge, TextEntry, text_key, TE_EDIT, TE_COMMIT,
                              TE_CANCEL)
